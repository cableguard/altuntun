...
impl RateLimiter {
    ...
    pub fn verify_packet<'a, 'b>(
        ...
        if let Packet::HandshakeInit(HandshakeInit { sender_session_index, .. })
        | Packet::HandshakeResponse(HandshakeResponse { sender_session_index, .. }) = packet
        {
            ...
            if self.is_under_load() {
                ...
                if verify_slices_are_equal(&computed_mac2[..16], mac2).is_err() {
                    let cookie_packet = self
                        .produce_cookie_reply(sender_session_index, cookie, mac1, dst)
